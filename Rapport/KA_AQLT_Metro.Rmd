...
1. Changer le répertoire de travail (`project_directory`)

2. Choisir si l'on veut mettre la couleur  (booléen `activerCouleur`). Cette fonctionnalité n'est opérationnelle que sur les ordinateurs Windows. 
...
<!--  Kim : demander a Alain de numéroter automatiquement figures --> 
La figure 1 décrit les relations entre toutes ces classes.
...
```{r, include=TRUE, fig.align="center", fig.cap=c("Liens entre les classes du projet"), echo=FALSE}
knitr::include_graphics("./img/Diagramme_classes/diagramme_classes.pdf")
```
